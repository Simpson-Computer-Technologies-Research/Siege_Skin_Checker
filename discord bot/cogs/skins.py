import discord, requests, json, os, base64
...
class Skins(commands.Cog):
    ...
    # MAIN FUNCTION
    # //////////////////
    def get_skins(self, username: str):
        ...
        if "message" in r.json() and "Ticket is expired" in r.json()["message"]:
            self.login(); print("[+] New Auth Token Generated")
    ...
    async def skins(self, ctx: commands.Context, name: str):
        embed=discord.Embed(title=f"{name[0].upper()}{name[1:]}", url=f'https://r6.tracker.network/profile/pc/{name}', color=65535)
